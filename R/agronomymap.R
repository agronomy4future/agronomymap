...
agronomymap= function(data,
  ...
  if (nrow(interp_points) < 4) {
    gx= seq(min(X), max(X), length.out = grid_res)
    gy= seq(min(Y), max(Y), length.out = grid_res)
    ...
    grid_df$VALUE= imputeTS::na_interpolation(
      as.vector(grid_data$z), option = "spline"
    )
  }
  ...
  p= ggplot() +
    ...
    theme(
      legend.title= element_text(size= label_size, margin=margin(b= 15)),
  ...
  p= p +
    scale_x_continuous(
      breaks= x_unique,
      labels= sort(unique(data[[x_name]]))
    ) +
    scale_y_continuous(
      breaks= y_unique,
      labels= sort(unique(data[[y_name]]))
    )
